...
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = os.getcwd() + r'\bigprojstt-2775effac8a3.json'

# text_path = os.getcwd() + '/models/function/text/script.txt'
...
def transcribe_gcs_with_word_time_offsets(gcs_uri, language, writepath, filename):
    ...
    # 음성 적응 요청 단어
    speech_context = ['제약사항', '음미', '를 통하여', '다양한 함수와', '현재 경로를', '에 대해', '로컬', 'read', '이때', 'get', 'current', 'work', 'directory']
    
    # 입력 영상 인코딩 방식, 샘플링 개수 맞춰줄것, timeoffset 옵션 true
    config = speech.RecognitionConfig(
        encoding=speech.RecognitionConfig.AudioEncoding.LINEAR16,
        sample_rate_hertz=44100,
        language_code=language,
    ...
    text_file = open(writepath + filename + '_script.txt', 'w')
...
def audio_to_text(filepath, writepath, filename):            
    ## 저장소에 파일 업로드부분
    ...
    # 음원 추출 경로
    speech_file = filepath + filename + ".wav"
    blob.upload_from_filename(speech_file)


    gsc_uri = 'gs://bigproj_stt_bucket/temp1' # 클라우드 버킷 설정 경로

    transcribe_gcs_with_word_time_offsets(gsc_uri, 'ko-KR', writepath, filename)