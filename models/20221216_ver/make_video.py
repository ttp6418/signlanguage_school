...
class NLP:
    def __init__(self):
        self.komoran = Komoran(userdic=os.getcwd() + '/models/20221216 ver/my_dict.txt')
        self.pr = StopWord()
        pass
...
def make_wav(file_from, file_to):
    clip = mp.VideoFileClip(file_from)
    clip.audio.write_audiofile(file_to)
    # 스테레오 음원을 모노음원으로 변경
    sound = AudioSegment.from_wav(file_to)
    sound = sound.set_channels(1)
    sound.export(file_to, format='wav')
...
def make_video(file_from, file_to):
    clip = mp.VideoFileClip(file_from)
    clip.audio.write_audiofile(file_to)

    r = sr.Recognizer()
    with sr.AudioFile(file_to) as source:
    ...
    df= pd.read_csv(os.getcwd() + '/models/20221216 ver/'+'Preprocessed_crawl.csv', encoding = "cp949")   #수화영상이 포함된 csv파일
    words = df['kor']
    url=[]
    for i in words_new:
        word=i
    ...
    final_clip.speedx(2).write_videofile(file_to)

if __name__ == "__main__":
	print('make_video.py')

make_video(os.getcwd() + '/models/20221216 ver/opencv_0.mp4', os.getcwd() + '/models/20221216 ver/opencv_0.wav')